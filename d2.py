...
while True:
    try:
        ...
        if connectcounter >= 15:
            print('Cannot connect to phone at', sourceip, ':', e)
            sock.close()
            sys.exit(1)
        print('Waiting for phone... attempt', connectcounter, str(e))
        sock.close()
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    ...
    try:
        ...
        if err == errno.EAGAIN or err == errno.EWOULDBLOCK:
            try:
                ...
                err = e.args[0]
                if err == errno.EAGAIN or err == errno.EWOULDBLOCK:
                    processrunning = os.popen('ps aux').read()
                    if player_select == 2 and 'h264.bin' not in processrunning:
                        launchplayer(player_select)
                        sleep(0.01)
                    elif player_select == 0 and 'gst-launch-1.0' not in processrunning:
                        print(f"[watchdog] GStreamer pipeline not running — restarting (watchdog={watchdog})")
                        launchplayer(player_select)
                        sleep(3)  # give pipeline time to appear in ps
                    else:
                        watchdog = watchdog + 1
                        if watchdog % 500 == 0:
                            print(f"[watchdog] pipeline running, tick {watchdog}")
                        sleep(0.01)
                        if watchdog >= 7000:
                            killall(True)
                            sleep(1)
